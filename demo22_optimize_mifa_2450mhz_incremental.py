...
from optimize_lib import run_stage,shrink_bounds_around_best, write_json, mm, _fmt_params_singleline_raw, OptLogger,local_pattern_search_ifa
...
def main():
    ...
    best_local, sum_local = local_pattern_search_ifa(
        start_parameters=p,
        optimize_parameters=bounds,
        init_step_frac=0.08,           # 8% of each bound span
        min_step_mm=0.05,              # stop when all steps < 0.05 mm
        shrink=0.5,
        max_rounds=40,
        bandwidth_target_db=10.0,      # optimize ≥ -10 dB over a band (optional)
        bandwidth_span=(2.3e9, 2.6e9),
        solver_name="CUDSS",
        timeout=600.0,
